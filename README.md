...
| 단계별 검산·복구 | 표가 둘 이상인 유적은 표를 하나 넣을 때마다 임시 저장본을 다시 확인합니다. 유적 하나가 끝나면 한글 작업 창을 새로 시작해 사진·클립보드 메모리 누적을 끊고, 실패 항목은 기록한 뒤 다음 유적을 처리합니다. |
...
분리 분석은 본문 전체를 훑지 않고 `rhwp`로 문서의 실제 표 구조를 읽습니다. 표가 둘 이상인 묶음은 각 표를 넣을 때마다 임시 저장본의 제목 표와 표 개수를 확인합니다. 최종 저장본도 다시 검산하므로, 빈 파일·사진 표 누락을 정상 결과로 처리하지 않습니다.
...
실패한 유적이 있어도 나머지는 계속 저장합니다. 작업 결과와 실패 이유는 결과 폴더의 `hwp_split_v42_journal.json`에 남으며, 같은 원본으로 다시 실행하면 검산을 통과한 파일은 건너뛰고 실패 항목만 다시 시도합니다. 안정성을 위해 표 분리 모드는 유적 하나가 끝날 때마다 한글 작업 창을 새로 시작하므로, 대량 문서에서는 이전 버전보다 시간이 더 걸릴 수 있습니다.
