...
| 단계별 검산·복구 | 표가 둘 이상인 유적은 표를 하나 넣을 때마다 임시 저장본을 다시 확인합니다. 실패 항목은 기록하고 다음 유적을 계속 처리합니다. |
...
실패한 유적이 있어도 나머지는 계속 저장합니다. 작업 결과와 실패 이유는 결과 폴더의 `hwp_split_v41_journal.json`에 남으며, 같은 원본으로 다시 실행하면 검산을 통과한 파일은 건너뛰고 실패 항목만 다시 시도합니다.
